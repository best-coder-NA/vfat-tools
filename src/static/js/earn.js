...
function pairmatch(p, t0, t1) {
  return (
    (p.token0.symbol.toLowerCase() == t0.toLowerCase() || p.token1.symbol.toLowerCase() == t0.toLowerCase()) &&
...
async function main() {
  ...
  const SNOWGLOBE_ABI = [
    ...
    {type: 'function', stateMutability: 'nonpayable', outputs: [], name: 'depositAll', inputs: []},
    ...
    {type: 'function', stateMutability: 'nonpayable', outputs: [], name: 'withdrawAll', inputs: []},
  ]
  const ICEQUEEN_ABI = [
    {
      type: 'constructor',
  ...
  const CRYSTAL_VAULT_ABI = [
    {
      type: 'constructor',
      stateMutability: 'nonpayable',
      inputs: [
        {type: 'address', name: '_iceQueen', internalType: 'address'},
        {type: 'address', name: '_snowball', internalType: 'address'},
        {type: 'address', name: '_pgl', internalType: 'address'},
    ...
    },
    {
      ...
      stateMutability: 'view',
      outputs: [
        {type: 'uint256', name: 'snowball', internalType: 'uint256'},
        {type: 'uint256', name: 'PGL', internalType: 'uint256'},
        {type: 'uint256', name: 'rewardCredit', internalType: 'uint256'},
        {type: 'uint256', name: 'rewardSnapshot', internalType: 'uint256'},
        {type: 'uint256', name: 'votes', internalType: 'uint256'},
        {type: 'uint256', name: 'thawTimestamp', internalType: 'uint256'},
      ],
      name: 'accounts',
      inputs: [{type: 'address', name: '', internalType: 'address'}],
    },
    {
      ...
      name: 'deposit',
      inputs: [
        {type: 'uint256', name: '_amountSnowball', internalType: 'uint256'},
        {type: 'uint256', name: '_amountPGL', internalType: 'uint256'},
      ],
    },
    {
      type: 'function',
      ...
      outputs: [],
      name: 'depositPGL',
      inputs: [{type: 'uint256', name: '_amountIn', internalType: 'uint256'}],
    },
    {
      type: 'function',
      stateMutability: 'nonpayable',
      outputs: [],
      name: 'depositSnowball',
      inputs: [{type: 'uint256', name: '_amountIn', internalType: 'uint256'}],
    },
    {
      type: 'function',
    ...
    },
    {
      type: 'function',
      stateMutability: 'view',
      outputs: [{type: 'address', name: '', internalType: 'contract IIceQueen'}],
      name: 'iceQueen',
      inputs: [],
    },
    {
    ...
    },
    {
      type: 'function',
      stateMutability: 'view',
      outputs: [{type: 'address', name: '', internalType: 'contract IPangolinPair'}],
      name: 'pgl',
      inputs: [],
    },
    {
  ...
  const STAKING_ABI = [
    ...
    {type: 'function', stateMutability: 'nonpayable', outputs: [], name: 'acceptOwnership', inputs: []},
    {
      type: 'function',
      stateMutability: 'view',
      outputs: [{type: 'uint256', name: '', internalType: 'uint256'}],
    ...
    {type: 'function', stateMutability: 'nonpayable', outputs: [], name: 'exit', inputs: []},
    {type: 'function', stateMutability: 'nonpayable', outputs: [], name: 'getReward', inputs: []},
    {
      type: 'function',
      stateMutability: 'view',
      outputs: [{type: 'uint256', name: '', internalType: 'uint256'}],
    ...
    },
    {
      ...
      outputs: [],
      name: 'setRewardsDuration',
      inputs: [{type: 'uint256', name: '_rewardsDuration', internalType: 'uint256'}],
    },
    {
      type: 'function',
      stateMutability: 'nonpayable',
  ...
  const withdrawPool8 = async function() {
    return stakingContract_withdraw({
      STAKING_ABI,
      STAKING_ADDR,
      S3F_ADDRESS,
      App,
  ...
  const STAKING_CONTRACT = new ethers.Contract(STAKING_ADDR, STAKING_ABI, signer)
  let snobTotalSupply,
    pendingSNOBTokensPool1,
    pendingSNOBTokensPool2,
    pendingSNOBTokensPool3,
    pendingSNOBTokensPool4,
    pendingSNOBTokensPool5,
    pendingSNOBTokensPool6,
    pendingSNOBTokensPool7,
    pendingSNOBTokensPool8
  ...
  await Promise.all([App.provider.getBlock(blockNumber), App.provider.getBlock(blockNumber - 20000)]).then(res => {
    currentBlock = res[0]
  ...
  // wallet info
  const claimableSnowballs =
    pendingGovReward / 1e18 +
    pendingSNOBTokensPool1 / 1e18 +
    pendingSNOBTokensPool2 / 1e18 +
    pendingSNOBTokensPool3 / 1e18 +
    pendingSNOBTokensPool4 / 1e18 +
    pendingSNOBTokensPool5 / 1e18 +
    pendingSNOBTokensPool6 / 1e18 +
    pendingSNOBTokensPool7 / 1e18 +
    pendingSNOBTokensPool8 / 1e18
  ...
  const snobPrice = prices['0xC38f41A296A4493Ff429F1238e030924A1542e50']
    ? prices['0xC38f41A296A4493Ff429F1238e030924A1542e50'].usd
    : 0
  ...
  $('#distribution_phase').append(
    `${blockNumber.toLocaleString()} / 3,065,000 (${(3065000 - blockNumber).toLocaleString()} blocks left)`
  )

  document.getElementById('wallet-copy').addEventListener('click', () => {
    navigator.clipboard.writeText(`${App.YOUR_ADDRESS}`).then(
      function() {
  ...
  let currentSPGLSUSHITokens,
    currentSPGLPNGTokens,
    currentSPGLETHTokens,
    currentSPGLUSDTTokens,
    currentSPGLLINKTokens,
    currentS3DTokens,
    currentS3FTokens,
    currentSNOBAVAXTokens
  let stakedPool1, stakedPool2, stakedPool3, stakedPool4, stakedPool5, stakedPool6, stakedPool7, stakedPool8
  let totalStakedSPGLSUSHI,
    totalStakedSPGLPNG,
    totalStakedSPGLETH,
    totalStakedSNOBAVAX,
    totalStakedSPGLUSDT,
    ...
    return Promise.all([
      ...
      .then(res => {
        ...
        return [
          ...
          <p class="m-0 font-size-16 font-weight-regular">${reserve0Owned.toFixed(3)} ${
            TOKEN_NAMES[token0Address]
          } / ${reserve1Owned.toFixed(3)} ${TOKEN_NAMES[token1Address]}  </p>
  ...
  if (stakedPool1.amount / 1e18 > 0) {
    let ret_1 = await calculateShare(
      snowglobeContract_1,
      SUSHI_AVAX_ADDR,
      stakedPool1.amount / 1e18,
      1e18,
      userPool1Percent
    )
  ...
  if (stakedPool3.amount / 1e18 > 0) {
    let ret_3 = await calculateShare(
      snowglobeContract_3,
      PNG_AVAX_ADDR,
      stakedPool3.amount / 1e18,
      1e18,
      userPool3Percent
    )
  ...
  if (stakedPool4.amount / 1e18 > 0) {
    let ret_4 = await calculateShare(
      snowglobeContract_4,
      ETH_AVAX_ADDR,
      stakedPool4.amount / 1e18,
      1e18,
      userPool4Percent
    )
  ...
  if (stakedPool5.amount / 1e18 > 0) {
    let ret_5 = await calculateShare(
      snowglobeContract_5,
      USDT_AVAX_ADDR,
      stakedPool5.amount / 1e18,
      1e6,
      userPool5Percent
    )
  ...
  if (stakedPool6.amount / 1e18 > 0) {
    let ret_6 = await calculateShare(
      snowglobeContract_6,
      LINK_AVAX_ADDR,
      stakedPool6.amount / 1e18,
      1e18,
      userPool6Percent
    )
  ...
  function pool(options) {
    ...
    if (options.total_staked && options.total_pgl) {
      ...
      ).toLocaleString()} sPGL </span>
        <span class="badge badge-pill font-size-12 px-5 px-sm-10 mx-5 font-weight-regular">${(
    ...
    if (options.user_pool_percent > 0) {
      ...
      var estimatedRate = `<div class="col-sm-12 col-md-2 align-items-center text-center snob-tvl pb-10 pb-md-0 mx-auto">
        ...
        <span class="badge badge-success font-size-12 px-5 px-sm-10 mx-10">${(
          ((snowballsPerBlock * options.pool_weight * options.user_pool_percent) / 100 / 1e18) *
          blocks24hrs
        ).toFixed(2)} SNOB <ion-icon name="trending-up-outline"></ion-icon></span>
      ...
      var earning = `<div class="col-sm-12 col-md-2 align-items-center text-center snob-tvl pb-10 pb-md-0">
        ...
        <p class="m-0 font-size-16 font-weight-regular">${(
          ((snowballsPerBlock * options.pool_weight * options.user_pool_percent) / 100 / 1e18) *
          15000
        ).toFixed(2)} SNOB </p>
        <p class="m-0 font-size-12">per day ($${(
          ((snowballsPerBlock * options.pool_weight * options.user_pool_percent) / 100 / 1e18) *
          15000 *
          snobPrice
        ).toFixed(2)})</p>
        ...
        <div class="card border-0 p-10 pl-20 pr-20 mt-5">
            <div class="row">
                ...
                <div onclick="toggleDetails('${
                  options.pool_nickname
                }');" class="col-sm-12 col-md-1 align-items-center text-center text-md-right snob-tvl pb-10 pb-md-0 mx-auto">
            ...
            <div id="details-${options.pool_nickname}" class="border-top mt-20 pt-10 pb-10" style="display: none;">
                <div class="row">
                    ...
                    <div class="col-sm-12 col-md-3 align-items-center text-center snob-tvl pb-10 pb-md-0">
                        ...
                        <span class="badge font-size-12 px-5 px-sm-10 mx-5 font-weight-regular">${options.pool_weight *
                          100}%</span>
                        <p class="m-0 font-size-12 pt-10"><ion-icon name="ellipse-outline"></ion-icon> SNOB per day</p>
                        <span class="badge font-size-12 px-5 px-sm-10 mx-5 font-weight-regular">${((snowballsPerBlock *
                          options.pool_weight) /
                          1e18) *
                          15000}</span>
                    </div>
            ...
            <div id="details-${options.pool_nickname}" class="border-top mt-20 pt-10 pb-10" style="display:none">
                ...
                <div class="row pt-20">
                    ...
                    <div class="col-sm-12 col-md-2 align-items-center text-center snob-tvl pb-10 pb-md-0">
                        ...
                        <p class="m-0 font-size-16 font-weight-regular">${(options.pending_tokens / 1e18).toFixed(
                          6
                        )}</p>
                    </div>
                    ${availableUnstake}
            ...
            <div class="row">
                ...
                <div class="col-sm-12 col-md-2 align-items-center text-center text-md-right snob-tvl pb-10 pb-md-0 mx-auto">
                    <a target="_blank" href="${
                      options.url
                    }" class="btn btn-primary btn-sm"><ion-icon name="link-outline"></ion-icon> Get PGL from Pangolin</a>
                </div>
  ...
  poolS3D({
    logo_token1:
      'https://x-api.snowball.network/assets/avalanche-tokens/0xba7deebbfc5fa1100fb055a87773e1e99cd3507a/logo.png',
    logo_token2:
      'https://x-api.snowball.network/assets/avalanche-tokens/0xaeb044650278731ef3dc244692ab9f64c78ffaea/logo.png',
    logo_token3:
      'https://x-api.snowball.network/assets/avalanche-tokens/0xde3a24028580884448a5397872046a019649b084/logo.png',
  ...
  pool({
    logo_token1:
      'https://x-api.snowball.network/assets/avalanche-tokens/0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7/logo.png',
    logo_token2:
      'https://x-api.snowball.network/assets/avalanche-tokens/0xb3fe5374f67d7a22886a0ee082b2e2f9d2651651/logo.png',
  ...
  pool({
    logo_token1:
      'https://x-api.snowball.network/assets/avalanche-tokens/0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7/logo.png',
    logo_token2:
      'https://x-api.snowball.network/assets/avalanche-tokens/0xf20d962a6c8f70c731bd838a3a388d7d48fa6e15/logo.png',
  ...
  pool({
    logo_token1:
      'https://x-api.snowball.network/assets/avalanche-tokens/0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7/logo.png',
    logo_token2:
      'https://x-api.snowball.network/assets/avalanche-tokens/0x60781c2586d68229fde47564546784ab3faca982/logo.png',
  ...
  poolSNOB({
    logo_token1:
      'https://x-api.snowball.network/assets/avalanche-tokens/0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7/logo.png',
    logo_token2:
      'https://x-api.snowball.network/assets/avalanche-tokens/0xc38f41a296a4493ff429f1238e030924a1542e50/logo.png',
  ...
  pool({
    logo_token1:
      'https://x-api.snowball.network/assets/avalanche-tokens/0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7/logo.png',
    logo_token2:
      'https://x-api.snowball.network/assets/avalanche-tokens/0x39cf1bd5f15fb22ec3d9ff86b0727afc203427cc/logo.png',
  ...
  function poolS3F(options) {
    ...
    poolSize = ''
    if (options.total_staked) {
      let poolSize = `<span class="badge badge-pill font-size-12 px-5 px-sm-10 mx-5 font-weight-regular">${(
        options.total_staked / 1e18
      ).toLocaleString()} S3F </span>`
    }
    let estimatedRate = ''
    let poolShare = ''
    let earning = ''
    ...
    if (options.user_pool_percent > 0) {
      ...
      estimatedRate = `<div class="col-sm-12 col-md-2 align-items-center text-center snob-tvl pb-10 pb-md-0 mx-auto">
        <p class="m-0 font-size-12"> Estimated Rate</p>
        <span class="badge badge-success font-size-12 px-5 px-sm-10 mx-10">${(
          (2666 * options.user_pool_percent) /
          100
        ).toFixed(2)} SNOB <ion-icon name="trending-up-outline"></ion-icon></span>
        <p class="m-0 font-size-12">per day ($${(
          ((2666 * options.user_pool_percent) / 100) *
          options.snobPrice
        ).toFixed(2)})</p>
        </div>`
      ...
      earning = `<div class="col-sm-12 col-md-2 align-items-center text-center snob-tvl pb-10 pb-md-0">
        <p class="m-0 font-size-12"><ion-icon name="pie-chart-outline"></ion-icon> You are earning</p>
        <p class="m-0 font-size-16 font-weight-regular">${((2666 * options.user_pool_percent) / 100).toFixed(
          2
        )} SNOB </p>
        <p class="m-0 font-size-12">per day ($${(
          ((2666 * options.user_pool_percent) / 100) *
            ...
            <div class="row">
                <div class="col-sm-12 col-md-3 align-items-center d-flex pb-10 pb-md-0">
                    <div id="pooltokens-3sd" class="align-items-center d-flex mx-auto mx-md-0 ">
                        <img class="rounded-circle" width="48" src="${options.logo_token1}" alt="${options.pool_name}">
                        <img class="rounded-circle" width="48" src="${options.logo_token2}" alt="${options.pool_name}">
                        <img style="background-color: white" class="rounded-circle" width="48" src="${
                          options.logo_token3
                        }" alt="${options.pool_name}">
                        <h6 class="pl-10 m-0">${options.pool_name}</h6>
                    </div>
                </div>
                <div class="col-sm-12 col-md-1 align-items-center text-center snob-tvl pb-10 pb-md-0 ${
  ...
  async function renderPoolS3F({STAKING_CONTRACT, App, SNOB_TOKEN, S3F_TOKEN}) {
...
const snowglobeContract_approve = async function(chefAbi, chefAddress, stakeTokenAddr, App) {
  ...
  if (allowedTokens / 1e18 == ethers.constants.MaxUint256 / 1e18) {
    ...
    snobMessage(
      ...
      `Already approved . <br>You can use the deposit/withdrawals options`,
      `checkmark-circle-outline`,
      `success`,
      false,
      `ok`,
    ...
    allow = STAKING_TOKEN.approve(chefAddress, ethers.constants.MaxUint256)
      ...
      .catch(function() {
        ...
        snobMessage(
          ...
          `Approval failed . Please check your Metamask Wallet`,
          `close-circle-outline`,
          `danger`,
          false,
          `ok`,
...
const snowglobeContract_stake = async function(chefAbi, chefAddress, poolIndex, stakeTokenAddr, App) {
  ...
  if (allowedTokens / 1e18 == 0) {
        ...
        CHEF_CONTRACT.depositAll()
          .then(function(t) {
            App.provider.waitForTransaction(t.hash).then(function() {
              ...
              snobMessage(
                `Tokens deposit`,
                `Tokens deposited. We will refresh the browser in 5 seconds to see balance.`,
                `checkmark-circle-outline`,
                `success`,
                false,
                `ok`
    ...
    snobMessage(
      `Oops! Failed`,
      `Deposit Failed. You have no tokens to stake`,
      `close-circle-outline`,
      `danger`,
      false,
      `ok`,
...
const snowglobeContract_withdraw = async function(chefAbi, chefAddress, poolIndex, stakeTokenAddr, App) {
  ...
  if (currentTokens / 1e18 > 0) {
        ...
        CHEF_CONTRACT.withdrawAll()
          .then(function(t) {
            App.provider.waitForTransaction(t.hash).then(function() {
              ...
              snobMessage(
                `Withdrawn Tokens`,
                `Tokens Withdrawn. We will refresh the browser in 5 seconds to see balance.`,
                `checkmark-circle-outline`,
                `success`,
                false,
                `ok`
    ...
    snobMessage(
      `Withdrawn Tokens`,
      `Withdrawn failed . You have no tokens to withdraw`,
      `close-circle-outline`,
      `danger`,
      false,
      `ok`,
        ...
        CHEF_CONTRACT.withdraw(poolIndex, 1)
          .then(function(t) {
            App.provider.waitForTransaction(t.hash).then(function() {
              ...
              snobMessage(
                `Withdrawn Tokens`,
                `Rewards claimed. We will refresh the browser in 5 seconds to see balance.`,
                `checkmark-circle-outline`,
                `success`,
                false,
                `ok`
